...
export const AddProjectModal: FC<IModalProps> = () => {
  ...
  const selectedUserId = useJiraStore((store) => store.selectedJiraUser );
	const setSelectedUser = useJiraStore(
		(state) => state.setSelectedJiraUser,
	);

  const userSelectOptions = data?.filter(({ accountType }) => accountType !== "app").map((user) => (
          ...
          <div className="grid gap-4 py-4">
            ...
            <Interceptor status={status} errorMessage={error?.message}>
                ...
                <Select
								...
								value={
									data?.some(
										(user) => String(user.accountId) === selectedUserId,
									)
										? selectedUserId
										: undefined
								}
